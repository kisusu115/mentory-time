import type { LectureEntry, NormalizedEntry, DetailInfo, LectureListEntry, NormalizedListEntry, LectureListStatus } from './types'
...
export function parseLectureListPage(doc: Document): LectureListEntry[] {
  ...
  rows.forEach((row) => {
    ...
    const { lectureDate, lectureStartTime, lectureEndTime } = parseLectureListDateTime(cells[3])
...
export function normalizeListEntry(entry: LectureListEntry): NormalizedListEntry {
