import { SWMAESTRO_LOGIN_URL } from "../lib/swmaestro";
  ...
  chrome.tabs
    ...
    .then((tab) => {
      ...
      if (url.includes(DETAIL_URL_PATTERN)) {
        ...
        if (newPending) {
          chrome.runtime
            .sendMessage({ type: "DETAIL_PAGE_DETECTED", payload: newPending })
            .catch(() => {});
        } else {
          chrome.runtime
            .sendMessage({ type: "DETAIL_PAGE_CLEARED", payload: null })
            .catch(() => {});
        }
...
chrome.runtime.onMessage.addListener(
  (
    ...
    if (message.type === "OPEN_SWMAESTRO_TAB") {
      const url = message.payload?.url ?? SWMAESTRO_LOGIN_URL;
      chrome.tabs
        ...
        .catch((error: unknown) => {
          const msg =
            error instanceof Error ? error.message : "TAB_CREATE_FAILED";
    ...
    if (message.type === "APPLY_COMPLETE" && sender.tab?.id) {
      ...
      const broadcast = () => {
        chrome.runtime
          .sendMessage({ type: "HISTORY_PAGE_DETECTED", payload: null })
          .catch(() => {});
      };
    ...
    if (message.type === "PAGE_DETECTED" && sender.tab?.id) {
      ...
      if (isDetail) {
        const qustnrSn =
          new URL(message.payload!.url).searchParams.get("qustnrSn") ?? "";
